...
static void kvm_mmu_free_shadow_page(struct kvm *kvm, struct kvm_mmu_page *sp)
{
	 ...
	 * Root Secure-EPT page has NULL private_sp.  Non-root Secure_EPT page
	 * has non-NULL private_sp.
	 */
	if (is_private_sp(sp) && !kvm_mmu_private_sp(sp) &&
...
struct shadow_page_caches {
	...
	struct kvm_mmu_memory_cache *private_sp_cache;
};
						      ...
						      union kvm_mmu_page_role role)
{
	...
	if (is_private)
		kvm_mmu_alloc_private_sp(vcpu, caches->private_sp_cache, sp);
...
int kvm_mmu_max_mapping_level(struct kvm *kvm,
			      ...
			      int max_level)
{
	return __kvm_mmu_max_mapping_level(kvm, slot, gfn, max_level, PG_LEVEL_NONE);
}
...
static void split_private_spte(struct kvm *kvm, u64 *sptep, u64 old_spte)
{
	...
	caches = (struct shadow_page_caches) {
		...
		.private_sp_cache = &kvm->arch.split_private_sp_cache,
	};
	...
	p = kvm_mmu_memory_cache_alloc(&kvm->arch.split_private_sp_cache);
	if (!static_call(kvm_x86_split_private_spte)(kvm, sp->gfn, sp->role.level, p))
				   ...
				   struct kvm_page_fault *fault)
{
	 ...
	 */
	if (fault->slot && fault->is_private != private_exist) {
...
static void mmu_free_vm_memory_caches(struct kvm *kvm)
{
	...
	kvm_mmu_free_memory_cache(&kvm->arch.split_private_sp_cache);
}
...
static bool need_topup_split_caches_or_resched(struct kvm *kvm)
{
	...
	return need_topup(&kvm->arch.split_desc_cache, SPLIT_DESC_CACHE_MIN_NR_OBJECTS) ||
	       ...
	       need_topup(&kvm->arch.split_private_sp_cache, 1);
}
...
static int topup_split_caches(struct kvm *kvm)
{
	...
	return kvm_mmu_topup_memory_cache(&kvm->arch.split_private_sp_cache, 1);
}
...
static struct kvm_mmu_page *shadow_mmu_get_sp_for_split(struct kvm *kvm, u64 *huge_sptep)
{
	...
	caches.private_sp_cache = &kvm->arch.split_private_sp_cache; /* FIXME. move to a proper patch*/
...

{
	...
	if (!static_call(kvm_x86_split_private_spte)(kvm, huge_sp->gfn,
						     huge_sp->role.level, p))
		kvm_mmu_init_private_sp(huge_sp, p);
	else {
		WARN_ON(1);
		free_page((unsigned long)p);
	}
