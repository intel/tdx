...
static inline bool is_empty_cmr(struct cmr_info *cmr)
{
	return (!cmr->base && !cmr->size);
}
...
/* Sanitize CMRs generated by BIOS */
static int sanitize_cmrs(void)
{
	 ...
	 * of CMR_INFO entries. The CMRs are sorted from the lowest base
	 * address to the highest base address, and they are non-overlapping.
	 *
	 * So BIOS may generate less valid CMRs than MAX_CMRS(32), but
	 * still reports 32 CMRs have been written, in which case the
	 * CMRs after the last valid one are empty.  Manually skip those
	 * empty CMRs.
	 */
	for (i = 0; i < tdx_cmr_num; i++) {
		...
		/* It is a TDX firmware bug if CMR is not valid */
		if (!is_valid_cmr(cmr)) {
			pr_err("TDX firmware bug: Invalid CMR: idx %d, base 0x%llx, size 0x%llx",
					i, cmr->base, cmr->size);
			return -EFAULT;
		}
	...
	/*
	 * Trim all tail empty CMRs.  BIOS must generate at least one
...
/* Get TDSYSINFO_STRUCT and CMR_INFO array */
static int get_tdx_sysinfo(void)
{
	u64 tdsysinfo_sz, cmr_num;
	u64 ret;

	ret = tdh_sys_info(&tdx_tdsysinfo, tdx_cmr_array,
			&tdsysinfo_sz, &cmr_num);
	if (ret) {
		SEAMCALL_ERR_WARN("TDH.SYS.INFO", ret);
		return -EFAULT;
	}

	if (WARN_ON(tdsysinfo_sz > sizeof(tdx_tdsysinfo)) ||
	...
	pr_info("TDX module: vendor_id 0x%x, major_version %u, minor_version %u, build_date %u, build_num %u",
			tdx_tdsysinfo.vendor_id, tdx_tdsysinfo.major_version,
			tdx_tdsysinfo.minor_version, tdx_tdsysinfo.build_date,
