		...
		unsigned long start_pfn, unsigned long end_pfn, int nid)
{
	...
	/*
	 * Due to some reasons, CMRs generated by BIOS won't include
	 * memory below 1MB.  Before constructing TDMRs, all TDX memory
	 * blocks are checked against CMRs to see whether they are fully
	 * covered by CMRs.  Skip the memory below 1MB so that the CMR
	 * check above won't fail.  And skipping memory below 1MB is
	 * fine for TDX, since memory below 1MB is reserved during
	 * setup_arch() and won't be used by page allocator anyway.
	 */
