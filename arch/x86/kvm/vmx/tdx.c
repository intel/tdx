...
static int tdx_emulate_mmio(struct kvm_vcpu *vcpu)
{
	...
	gpa = gpa & ~gfn_to_gpa(kvm_gfn_direct_mask(vcpu->kvm));
...
static int tdx_td_init(struct kvm *kvm, struct kvm_tdx_cmd *cmd)
{
	...
	if (td_params->exec_controls & TDX_EXEC_CONTROL_MAX_GPAW)
		kvm->arch.gfn_direct_mask = gpa_to_gfn(BIT_ULL(51));
	else
		kvm->arch.gfn_direct_mask = gpa_to_gfn(BIT_ULL(47));
...
int __init tdx_hardware_setup(struct kvm_x86_ops *x86_ops)
{
	...
	x86_ops->reflect_link_spt = tdx_sept_link_private_spt;
	x86_ops->reflect_free_spt = tdx_sept_free_private_spt;
	x86_ops->reflect_set_spte = tdx_sept_set_private_spte;
	x86_ops->reflect_remove_spte = tdx_sept_remove_private_spte;
