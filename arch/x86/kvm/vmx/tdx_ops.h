...
static inline u64 __kvm_seamcall_sept(u64 op, u64 rcx, u64 rdx, u64 r8, u64 r9,
				    struct tdx_module_output *out)
{
				...
				struct tdx_module_output *out)
{
	...
	return __kvm_seamcall_sept(op, rcx, rdx, r8, r9, out);
}
...
static inline u64 tdh_mng_addcx(hpa_t tdr, hpa_t addr)
{
	...
	r = kvm_seamcall(TDH_MNG_ADDCX, addr, tdr, 0, 0, NULL);
	if (!r)
				   ...
				   hpa_t source, struct tdx_module_output *out)
{
	...
	r = kvm_seamcall_sept(TDH_MEM_PAGE_ADD, gpa | level, tdr, hpa, source, out);
	if (!r)
...
static inline u64 tdh_mem_sept_rd(hpa_t tdr, gpa_t gpa, int level,
				  struct tdx_module_output *out)
{
	return kvm_seamcall_sept(TDH_MEM_SEPT_RD, gpa | level, tdr, 0, 0, out);
}
...
static inline u64 tdh_vp_addcx(hpa_t tdvpr, hpa_t addr)
{
	...
	tdx_clflush_page(addr, PG_LEVEL_4K);
	r = kvm_seamcall(TDH_VP_ADDCX, addr, tdvpr, 0, 0, NULL);
	if (!r)
		tdx_set_page_np(addr);
	return r;
}
				   ...
				   struct tdx_module_output *out)
{
	...
	tdx_clflush_page(hpa, tdx_sept_level_to_pg_level(level));
	r = kvm_seamcall_sept(TDH_MEM_PAGE_AUG, gpa | level, tdr, hpa, 0, out);
	if (!r)
		tdx_set_page_np_level(hpa, level);
	return r;
}
				      ...
				      struct tdx_module_output *out)
{
	 ...
	 */
	return __kvm_seamcall_sept(TDH_MEM_RANGE_BLOCK, gpa | level, tdr, 0, 0, out);
}
...
static inline u64 tdh_mng_create(hpa_t tdr, int hkid)
{
	...
	r = kvm_seamcall(TDH_MNG_CREATE, tdr, hkid, 0, 0, NULL);
	if (!r)
		tdx_set_page_np(tdr);
	return r;
}
...
static inline u64 tdh_vp_create(hpa_t tdr, hpa_t tdvpr)
{
	...
	r = kvm_seamcall(TDH_VP_CREATE, tdvpr, tdr, 0, 0, NULL);
	if (!r)
...
static inline u64 tdh_mem_page_promote(hpa_t tdr, gpa_t gpa, int level,
				       struct tdx_module_output *out)
{
	return kvm_seamcall_sept(TDH_MEM_PAGE_PROMOTE, gpa | level, tdr, 0, 0, out);
}
