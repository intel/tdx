...
static struct tdx_sys_info tdx_sys_info;
...
static int init_tdx_module(void)
{
	...
	ret = get_tdx_sys_info(&tdx_sys_info);
	if (ret)
		return ret;

	print_basic_sys_info(&tdx_sys_info);

	/* Check whether the kernel can support this module */
	ret = check_features(&tdx_sys_info);
	if (ret)
		return ret;
	...
	ret = alloc_tdmr_list(&tdx_tdmr_list, &tdx_sys_info.tdmr);
	if (ret)
		goto err_free_tdxmem;

	/* Cover all TDX-usable memory regions in TDMRs */
	ret = construct_tdmrs(&tdx_memlist, &tdx_tdmr_list,
			      &tdx_sys_info.tdmr, &tdx_sys_info.cmr);
...
const struct tdx_sys_info *tdx_get_sysinfo(void)
{
	...
	if (tdx_module_status == TDX_MODULE_INITIALIZED)
		p = (const struct tdx_sys_info *)&tdx_sys_info;
	mutex_unlock(&tdx_module_lock);
...
u64 tdh_mem_page_add(u64 tdr, u64 gpa, u64 hpa, u64 source, u64 *rcx, u64 *rdx)
{
	...
	/*
	 * Additional error information (ditto for all TDH.MEM.PAGE.xx,
	 * TDH.MEM.RANGE.xx and and TDH.MEM.SEPT.xx SEAMCALL leafs):
	 *
	 *  - RCX: secure EPT entry content where failed
	 *  - RDX: secure EPT entry level and state where failed
	 */
	*rcx = args.rcx;
	*rdx = args.rdx;
...
u64 tdh_mng_wr(u64 tdr, u64 field, u64 data, u64 mask)
{
	...
	return seamcall(TDH_MNG_RD, &args);
}
EXPORT_SYMBOL_GPL(tdh_mng_wr);
...
u64 tdh_mem_sept_remove(u64 tdr, u64 gpa, u64 level, u64 *rcx, u64 *rdx)
{
	...
	ret = tdx_seamcall_sept(TDH_MEM_SEPT_REMOVE, &args);
...
EXPORT_SYMBOL_GPL(tdh_mem_sept_remove);
...
u64 tdh_mem_range_unblock(u64 tdr, u64 gpa, u64 level, u64 *rcx, u64 *rdx)
{
	...
	ret = tdx_seamcall_sept(TDH_MEM_RANGE_UNBLOCK, &args);
...
EXPORT_SYMBOL_GPL(tdh_mem_range_unblock);
...
u64 tdh_phymem_page_wbinvd_tdr(u64 tdr)
{
	...
	args.rcx = tdr | (tdx_global_keyid << boot_cpu_data.x86_phys_bits);
