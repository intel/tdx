				     ...
				     int tdmr_num)
{
	...
	/*
	 * Walk over all e820 RAM entries, and mark all holes
	 * as reserved.
	 */
	rsvd_idx = 0;
	...
	e820_for_each_ram_entry(e820_table, i, entry, start, end) {
		 ...
		 * used because the first e820 entry might be split into
		 * two TDMRs.  In that case the start address of the
		 * first e820 entry is smaller than the start of the
		 * second TDMR.
		 */
		if (start <= prev_end) {
			prev_end = end;
