...
#define TDX_HYPERCALL_VENDOR_KVM		0x4d564b2e584454 /* TDX.KVM */
	...
	if (nr == TDX_HYPERCALL_VENDOR_KVM) {
		/*
		 * TODO: once the the guest ABI change is done, remove this
		 * ABI support.
			   ...
			   enum exit_fastpath_completion fastpath)
{
	...
	if (unlikely(exit_reason.non_recoverable || exit_reason.error)) {
		kvm_pr_unimpl("TD exit due to %s(0x%llx), Exit Reason %d qual 0x%lx ext 0x%lx gpa 0x%lx intr 0x%lx\n",
...
static void tdx_get_exit_info(struct kvm_vcpu *vcpu, u32 *reason,
