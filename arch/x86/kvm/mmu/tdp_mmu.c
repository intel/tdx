					  ...
					  u64 new_spte)
{
	...
	__handle_changed_spte(
		kvm, iter->as_id, iter->gfn, is_private_spte(iter->sptep),
			       ...
			       bool record_acc_track, bool record_dirty_log)
{
	...
	__handle_changed_spte(kvm, as_id, gfn, is_private_spte(sptep),
			      old_spte, new_spte, level, false);

	if (record_acc_track)
		handle_changed_spte_acc_track(old_spte, new_spte, level);
	if (record_dirty_log)
		handle_changed_spte_dirty_log(kvm, as_id, gfn,
					      is_private_spte(sptep),
...
{
	struct kvm_mmu_page *sp;
	bool is_private = is_private_spte(iter->sptep);
	...
	struct kvm *kvm, struct tdp_iter *iter, bool allow_private)
{
	...
	WARN_ON(tdp_iter_is_private(iter));
	if (allow_private) {
			...
			gfn_t *startp, gfn_t end, bool allow_private)
{
	...
	lockdep_assert_held_write(&kvm->mmu_lock);
	WARN_ON(start != kvm_gfn_unalias(kvm, start));
	WARN_ON(end != kvm_gfn_unalias(kvm, end));
