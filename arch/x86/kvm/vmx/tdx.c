...
#define pr_err_skip_ud(_x) \
	pr_err_once("Skip #UD injection for " _x " due to it's not supported in this TDX version\n")
...
int tdx_vcpu_check_cpuid(struct kvm_vcpu *vcpu, struct kvm_cpuid_entry2 *e2, int nent)
{
	...
	for (i = 0; i < tdx_info->num_cpuid_config; i++) {
		const struct tdx_cpuid_config *config = &tdx_info->cpuid_configs[i];
...
#define MD_FID_OFFSET(field, member)					\
	...
	struct {
		...
		{ TDX_MD_FID_GLOBAL_TDCS_BASE_SIZE, &tdcs_base_size },
		{ TDX_MD_FID_GLOBAL_TDVPS_BASE_SIZE, &tdvps_base_size },
	};
	...
	for (i = 0; i < ARRAY_SIZE(md16); i++) {
		err = tdh_sys_rd16(md16[i].field, md16[i].data);
		if (err)
			goto error_sys_rd;
	}
