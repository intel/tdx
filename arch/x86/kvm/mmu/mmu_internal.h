...
static inline void kvm_mmu_alloc_private_spt(
	struct kvm_vcpu *vcpu, struct kvm_mmu_memory_cache *private_spt_cache,
...
static inline int kvm_alloc_private_spt_for_split(
					...
					struct kvm_mmu_page *sp)
{
	...
	return static_call(kvm_x86_link_private_spt)(
		kvm, sp->gfn, sp->role.level + 1, sp->private_spt);
}
#else
static inline void *kvm_mmu_private_spt(struct kvm_mmu_page *sp)
{
...
struct kvm_page_fault {
	...
	hva_t hva;
	bool map_writable;
	enum pg_level host_level; /* valid only for private memslot */
};
