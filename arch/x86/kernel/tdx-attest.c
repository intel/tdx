...
/*
 * tdx-attest.c - TDX guest attestation interface driver.
 *
...
static long tdg_attest_ioctl(struct file *filp, unsigned int cmd,
		unsigned long arg)
{
	u8 *reportdata;
	long ret = 0;
	...
	case TDX_CMD_GET_TDREPORT:
		...
		if (tdg_get_tdreport(data, virt_to_phys(reportdata))) {
			ret = -EIO;
			break;
		}
...
static int tdg_attest_mmap(struct file *filp, struct vm_area_struct *vma)
{
...
static int tdg_attest_open(struct inode *inode, struct file *filp)
{
	if (tdg_register_event_notify_handler(attestation_callback_handler))
		return -ENODEV;
...
static int tdg_attest_release(struct inode *inode, struct file *filp)
{
	tdg_unregister_event_notify_handler();
...
builtin_misc_device(tdg_attest_device);
