				     ...
				     enum pg_level level, kvm_pfn_t pfn)
{
	...
	if (likely(is_td_finalized(kvm_tdx))) {
		...
		if (err == (TDX_EPT_ENTRY_NOT_FREE | TDX_OPERAND_ID_RCX)) {
			/*
				      ...
				      enum pg_level level)
{
	...
	if (err == TDX_ERROR_SEPT_BUSY)
		return -EAGAIN;
	if (err == (TDX_GPA_RANGE_ALREADY_BLOCKED | TDX_OPERAND_ID_RCX))
		return -EAGAIN;
				       ...
				       enum pg_level level)
{
	...
	if (err == TDX_ERROR_SEPT_BUSY)
		return -EAGAIN;
	if (err == (TDX_GPA_RANGE_NOT_BLOCKED | TDX_OPERAND_ID_RCX))
		return -EAGAIN;
	if (err == (TDX_EPT_ENTRY_STATE_INCORRECT | TDX_OPERAND_ID_RCX)) {
		union tdx_sept_level_state level_state = {
			.raw = out.rdx,
...
int tdx_set_msr(struct kvm_vcpu *vcpu, struct msr_data *msr)
{
	if (tdx_has_emulated_msr(msr->index, true) ||
	    /*
	     * user space VMM must explicitly set to X2APIC mode as the initial
	     * reset value that is different from the conventional case.
	     */
...
static int __init tdx_module_setup(void)
{
	...
	tdx_info = (struct tdx_info) {
		...
		/*
		 * TDX module 1.0/2.0 have supported major and minor version,
		 * but seems TDX module 1.5 can't support minior version until
		 * now.
		 */
		.tdx_version = tdx_get_module_version(tdsysinfo->major_version,
