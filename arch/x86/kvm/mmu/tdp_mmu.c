...
static void handle_removed_pt(struct kvm *kvm, tdp_ptep_t pt, bool shared)
{
	...
	WARN_ON(is_private_sp(sp) && !kvm_mmu_private_spt(sp));
	if (is_private_sp(sp) &&
				  ...
				  union kvm_mmu_page_role role, bool shared)
{
	...
	WARN_ON(kvm_is_private_gpa(kvm, gfn_to_gpa(gfn)) != is_private);
	WARN_ON(was_private_zapped && !is_private);
	...
	if (is_private &&
		 ...
		 */
		WARN_ON(!shared && is_leaf && !is_private_zapped);
			       ...
			       bool record_acc_track, bool record_dirty_log)
{
	...
	WARN_ON(is_private_sptep(sptep) != kvm_is_private_gpa(kvm, gfn_to_gpa(gfn)));
	lockdep_assert_held_write(&kvm->mmu_lock);
