...
static bool __read_mostly clear_poison;
module_param_named(clear_poison, clear_poison, bool, 0644);
			      ...
			      bool map_private)
{
	...
	if (kvm_slot_can_be_private(slot)) {
		rcu_read_unlock();
		/*
		 * This case is handled by memfile callback.
		 * kvm_memfile_notifier_invalidate() => kvm_zap_gfn_range()
		 */
		return 0;
	}
	rcu_read_unlock();
			    ...
			    void *data)
{
	...
	kaddr = (unsigned long)vmap(&page, 1, 0, PAGE_KERNEL);
	if (!kaddr)
		return NOTIFY_DONE;
