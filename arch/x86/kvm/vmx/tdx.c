...
#undef REG
	...
	tdx->vp_enter_ret = __seamcall_saved_ret(TDH_VP_ENTER, &args);
...
static int tdx_complete_vmcall_map_gpa(struct kvm_vcpu *vcpu)
{
	...
	if (pi_has_pending_interrupt(vcpu) ||
		...
		tdvmcall_set_return_code(vcpu, TDVMCALL_RETRY);
		kvm_r11_write(vcpu, tdx->map_gpa_next);
		return 1;
	}
...
static int tdx_map_gpa(struct kvm_vcpu *vcpu)
{
	 ...
	 * TDVMCALL_INVALID_OPERAND for this case.
	 */
	if (!(vcpu->kvm->arch.hypercall_exit_enabled & BIT(KVM_HC_MAP_GPA_RANGE))) {
		ret = TDVMCALL_INVALID_OPERAND;
		goto error;
	}
