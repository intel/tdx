...
static bool vt_is_emulatable(struct kvm_vcpu *vcpu, void *insn, int insn_len)
{
	if (is_td_vcpu(vcpu))
		return false;
...
static void vt_set_efer(struct kvm_vcpu *vcpu, u64 efer)
{
	if (KVM_BUG_ON(is_td_vcpu(vcpu), vcpu->kvm))
		return;
...
{
	if (is_td_vcpu(vcpu))
		return tdx_get_exit_info(vcpu, info1, info2);

	return vmx_get_exit_info(vcpu, info1, info2);
}
...
static struct kvm_x86_ops vt_x86_ops __initdata = {
	...
	.is_emulatable = vt_is_emulatable,
