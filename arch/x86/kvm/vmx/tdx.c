...
int tdx_vm_init(struct kvm *kvm)
{
	...
	for_each_online_cpu(i) {
		 ...
		 * mutex.
		 */
		pkg = topology_physical_package_id(i);
...
#define TDX_MAP_GPA_SIZE_MAX   (16 * 1024 * 1024)
	 ...
	 */
       ...
       private_slot = kvm_slot_is_private(
	       gfn_to_memslot(kvm, kvm_gpa_unalias(kvm, gpa)));
...
static int handle_tdvmcall(struct kvm_vcpu *vcpu)
{
	...
	trace_kvm_tdx_hypercall(
		tdvmcall_exit_reason(vcpu), kvm_rcx_read(vcpu),
		kvm_r12_read(vcpu), kvm_r13_read(vcpu), kvm_r14_read(vcpu),
		kvm_rbx_read(vcpu), kvm_rdi_read(vcpu), kvm_rsi_read(vcpu),
		kvm_r8_read(vcpu), kvm_r9_read(vcpu), kvm_rdx_read(vcpu));
...
static void tdx_pin_gfn(struct kvm *kvm, gfn_t gfn, kvm_pfn_t pfn)
{
	/*
	 ...
	 * mmu_notifier and kvm memslot move/deletion.  Therefore gfn, pfn and
	...
	npinned = pin_user_pages_fast_only(
		uaddr, 1, FOLL_WRITE | FOLL_LONGTERM, &page);
	WARN_ON(npinned != 1);
...
static void tdx_track(struct kvm_tdx *kvm_tdx)
{
	...
	WRITE_ONCE(kvm_tdx->has_range_blocked, false);
	...
	struct kvm *kvm, const struct kvm_spte_change *change)
{
	...
	if (change->new.is_present) {
		...
		if (change->old.pfn == change->new.pfn) {
			...
			/* Page migration */
			tdx_pin_gfn(kvm, change->gfn, change->new.pfn);
...
static int tdx_td_init(struct kvm *kvm, struct kvm_tdx_cmd *cmd)
{
	...
	user_cpuid = (struct kvm_cpuid2 __user *)init_vm.cpuid;
