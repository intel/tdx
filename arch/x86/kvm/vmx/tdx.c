...
static int tdx_handle_dr(struct kvm_vcpu *vcpu)
{
	...
	return __vmx_handle_dr(vcpu, guest_debug_enabled);
}
			  ...
			  struct kvm_tdx_init_vm *init_vm)
{
	...
	if (td_params->xfam & TDX1_TD_XFAM_LBR) {
		pr_warn("TD doesn't support LBR. KVM needs to save/restore "
			"IA32_LBR_DEPTH properly.\n");
		return -ENOSYS;
	}

	if (td_params->xfam & TDX1_TD_XFAM_AMX) {
...
static void tdx_update_exception_bitmap(struct kvm_vcpu *vcpu)
{
	 ...
	 */
	if (!vcpu->arch.exception.emulate_inject_bp &&
	    (!vcpu->guest_debug || !(vcpu->guest_debug & intercept_bp))) {
		new_eb &= ~(1u << DB_VECTOR);
...
static void tdx_load_guest_debug_regs(struct kvm_vcpu *vcpu)
{
	 ...
	 * KVM_DEBUGREG_BP_ENABLED to avoid context switch
	 * DRs every time
	 */
...
static int do_write_private_memory(struct kvm *kvm, gpa_t addr, u64 *val)
{
	...
	if (TDX_ERR(err, TDH_MNG_WRMEM, &td_ret))
		return -EIO;
...
static int do_tdx_td_write_memory(struct kvm *kvm, gpa_t addr, u64 len,
				  u64 *complete_len, void __user *buf)
{
	u32 in_page_offset;
	u32 copy_len;
	...
	while (len > 0) {
		...
		while (round_len > 0) {
			...
			if (is_private) {
				...
				/*
				 * Refer the comments in do_tdx_td_read_memory()
				 * for reason we release the lock here
				 */
				read_unlock(&kvm->mmu_lock);
...
static int tdx_write_guest_memory(struct kvm *kvm, struct kvm_rw_memory *rw_memory)
{
	...
	ret = tdx_guest_memory_access_check(kvm, rw_memory);
	if (!ret) {
		ret = do_tdx_td_write_memory(kvm, rw_memory->addr, rw_memory->len,
					     &complete_len,
					     (void __user *)rw_memory->ubuf);
	}
...
static int buffer_show(struct seq_file *sfile, void *v)
{
	...
	if (v == SEQ_START_TOKEN) {
		if (buffer == buffer_trace)
			seq_printf(sfile, "------- trace buffer ------\n");
		else if (buffer == buffer_dump)
			seq_printf(sfile, "------- dump  buffer ------\n");
		else
			seq_printf(sfile, "------- emerg buffer ------\n");
	} else {
...
static int __init tdx_debugfs_init(void)
{
	...
	debugfs_create_file("print_severity", S_IRUSR | S_IWUSR,
			    tdx_seam, NULL, &print_severity_fops);
	debugfs_create_file("trace_target", S_IRUSR | S_IWUSR,
			    tdx_seam, NULL, &trace_target_fops);
	debugfs_create_file("emergency", S_IWUSR | S_IWUSR,
			    tdx_seam, NULL, &emergency_fops);

	debugfs_create_file("dump", S_IWUSR,
			    tdx_seam, NULL, &dump_fops);
	debugfs_create_file("buffer_trace", S_IRUSR,
			    tdx_seam, buffer_trace, &buffer_fops);
	debugfs_create_file("buffer_dump", S_IRUSR,
