...
static void tdx_reclaim_td_page(struct tdx_td_page *page)
{
	...
	if (page->pa) {
		free_page((unsigned long)__va(page->pa));
		page->pa = 0;
	}
}
...
static int __tdx_handle_exit(struct kvm_vcpu *vcpu, fastpath_t fastpath)
{
	...
	/*
	 * When TDX module see VMEXIT_REASON_FAILED_VMENTER_MC etc, TDH.VP.ENTER
	 * return with TDX_SUCCESS | exit_reason with failed_vmentry=1.
	 * Because TDX module maintains TD VMCS correctness, usually vmentry
	 * failure doesn't happen. In some corner cases it can happen. For
	 * example
...
free_tdcs:
	for (i = 0; i < tdx_caps.tdcs_nr_pages; i++) {
		if (!kvm_tdx->tdcs[i].pa)
			continue;
		free_page((unsigned long)__va(kvm_tdx->tdcs[i].pa));
	}
	kfree(kvm_tdx->tdcs);
	kvm_tdx->tdcs = NULL;
free_tdr:
...
free_tdvpx:
	...
	for (--i; i >= 0; i--)
		free_page((unsigned long)__va(tdx->tdvpx[i].pa));
	kfree(tdx->tdvpx);
	tdx->tdvpx = NULL;
free_tdvpr:
	free_page((unsigned long)__va(tdx->tdvpr.pa));
