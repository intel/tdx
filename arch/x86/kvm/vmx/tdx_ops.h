...
static inline u64 __tdx_seamcall_sept(u64 op, u64 rcx, u64 rdx, u64 r8, u64 r9,
				    struct tdx_module_args *out)
{
				...
				struct tdx_module_args *out)
{
	...
	return __tdx_seamcall_sept(op, rcx, rdx, r8, r9, out);
}
				      ...
				      struct tdx_module_args *out)
{
	 ...
	 */
	return __tdx_seamcall_sept(TDH_MEM_RANGE_BLOCK, gpa | level, tdr, 0, 0, out);
}
...
#define TDX_BUILD_SYS_RD_ACCESSORS(bits)				\
	static inline u64 tdh_sys_rd##bits(u64 field, u##bits *data)	\
	{								\
		struct tdx_module_args out;				\
		u64 err;						\
									\
		BUILD_BUG_ON_MSG(__builtin_constant_p(field) &&		\
				 TDX_MD_ELEMENT_SIZE_CODE(field) !=	\
				 TDX_MD_ELEMENT_SIZE_##bits##BITS,	\
				 "Invalid TDX metadata element size code"); \
		err = tdx_seamcall(TDH_SYS_RD, 0, field, 0, 0, &out);	\
		if (!err)						\
			*data = (u##bits)out.r8;			\
		return err;						\
	}

TDX_BUILD_SYS_RD_ACCESSORS(64)
TDX_BUILD_SYS_RD_ACCESSORS(32)
TDX_BUILD_SYS_RD_ACCESSORS(16)
