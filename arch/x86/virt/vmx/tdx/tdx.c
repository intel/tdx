...
#define READ_SYS_INFO(_field_id, _member, _size)			\
	ret = ret ?: read_sys_metadata_field(MD_FIELD_ID_##_field_id,	\
					&td_conf->_member, _size)

	READ_SYS_INFO(ATTRIBUTES_FIXED0, attributes_fixed0, 64);
	READ_SYS_INFO(ATTRIBUTES_FIXED1, attributes_fixed1, 64);
	READ_SYS_INFO(XFAM_FIXED0,	 xfam_fixed0, 64);
	READ_SYS_INFO(XFAM_FIXED1,       xfam_fixed1, 64);
	READ_SYS_INFO(NUM_CPUID_CONFIG,  num_cpuid_config, 16);

	for (u16 i = 0; i < td_conf->num_cpuid_config; i++) {
		READ_SYS_INFO(CPUID_CONFIG_LEAVES + i,
				cpuid_config_leaves[i], 64);
		READ_SYS_INFO(CPUID_CONFIG_VALUES + i * 2,
				cpuid_config_values[i].eax_ebx, 64);
		READ_SYS_INFO(CPUID_CONFIG_VALUES + i * 2 + 1,
				cpuid_config_values[i].ecx_edx, 64);
	}
	 ...
	 */
	READ_SYS_INFO(MAX_VCPUS_PER_TD, max_vcpus_per_td, 16);
	if (ret)
