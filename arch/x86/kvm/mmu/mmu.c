...
static void account_shadowed(struct kvm *kvm, struct kvm_mmu_page *sp)
{
	...
	WARN_ON(kvm_mmu_supports_shared_bit(kvm));
	gfn = sp->gfn;
...
static bool __kvm_mmu_zap_private_spte(struct kvm *kvm, u64 *sptep)
{
	...
	if (likely(!kvm_mmu_supports_shared_bit(kvm)))
		return false;
...
trace_get_page:
	...
	if (private)
		kvm_mmu_alloc_private_sp(
			vcpu, sp, level == vcpu->arch.mmu->shadow_root_level);
			    ...
			    u64 *spte, struct list_head *invalid_list)
{
	...
	if (is_shadow_present_pte(pte) || is_private_zapped_spte(pte)) {
		if (is_last_spte(pte, sp->role.level)) {
			...
			is_private_prohibit_spte(pte)) && is_mmio_spte(kvm, pte)) {
...
static void __direct_populate_nonleaf(struct kvm_vcpu *vcpu,
				...
				gfn_t *base_gfnp)
{
	bool is_private = kvm_is_private_fault(vcpu->kvm, fault);
...
static bool mmu_private_prohibit(struct kvm_vcpu *vcpu, gpa_t gpa_private)
{
	struct kvm_shadow_walk_iterator it;
	...
	return is_private_prohibit_spte(*it.sptep);
}
...
static int __direct_map(struct kvm_vcpu *vcpu, struct kvm_page_fault *fault)
{
	 ...
	 */
	if (is_private && is_private_prohibit_spte(*it.sptep))
		return RET_PF_RETRY;
	...
	pte_access = ACC_ALL;
	if (!is_private && kvm_mmu_supports_shared_bit(vcpu->kvm))
				...
				unsigned int access, int *ret_val)
{
	...
	if (unlikely(!fault->slot)) {
		...
		if (unlikely(!vcpu->kvm->arch.shadow_mmio_value)
		    && !kvm_mmu_supports_shared_bit(vcpu->kvm)) {
...
static int fast_page_fault(struct kvm_vcpu *vcpu, struct kvm_page_fault *fault)
{
	...
	do {
		...
		if (is_tdp_mmu(vcpu->arch.mmu))
			sptep = kvm_tdp_mmu_fast_pf_get_last_sptep(
				vcpu, fault->addr, &spte, is_private);
		else
...
static int mmu_alloc_direct_roots(struct kvm_vcpu *vcpu)
{
	...
	bool supports_shared_bit = kvm_mmu_supports_shared_bit(vcpu->kvm);
	hpa_t root;
	unsigned i;
	int r;
	...
	if (is_tdp_mmu_enabled(vcpu->kvm)) {
		if (supports_shared_bit && !VALID_PAGE(mmu->private_root_hpa)) {
		...
		WARN_ON_ONCE(supports_shared_bit);
...
static int mmu_alloc_shadow_roots(struct kvm_vcpu *vcpu)
{
	...
	if (mmu->root_level >= PT64_ROOT_4LEVEL) {
		root = mmu_alloc_root(
			vcpu, root_gfn, 0, vcpu->arch.mmu->shadow_root_level,
			false, false);
		mmu->root.hpa = root;
		goto set_root_pgd;
	}
...
static void kvm_mmu_zap_memslot(struct kvm *kvm, struct kvm_memory_slot *slot)
{
	...
	if (is_tdp_mmu_enabled(kvm)) {
		...
		if (kvm_mmu_supports_shared_bit(kvm)) {
...
static void kvm_mmu_invalidate_zap_pages_in_memslot(struct kvm *kvm,
			...
			struct kvm_page_track_notifier_node *node)
{
	if (memslot_update_zap_all || !kvm_mmu_supports_shared_bit(kvm))
	...
	for_each_rmap_spte(rmap_head, &iter, sptep) {
		...
		if (sp->role.direct && !kvm_is_reserved_pfn(pfn) &&
			sp->role.level < kvm_mmu_max_mapping_level(
				kvm, slot, kvm_gfn_unalias(kvm, sp->gfn),
				pfn, PG_LEVEL_NUM)) {
			pte_list_remove(kvm, rmap_head, sptep);
...
static int mmu_map_gpa(struct kvm_vcpu *vcpu, gfn_t gfn, bool allow_private)
{
	...
	if (allow_private) {
		...
		/* Clear SPTE_PRIVATE_PROHIBIT bit in 4K leaf spte. */
		...
		spte = *it.sptep;
		if (is_last_spte(spte, sp->role.level) &&
			is_private_prohibit_spte(spte)) {
			WARN_ON(it.level != PG_LEVEL_4K);
			WARN_ON(is_shadow_present_pte(*it.sptep));
			__mmu_spte_clear_track_bits(kvm, it.sptep, shadow_init_value);
		}
	} else {
		/* Zap and set SPTE_PRIVATE_PROHIBIT. */
		struct kvm_page_fault fault = {
			.addr = gfn_to_gpa(gfn),

			/* SPTE_PRIVATE_PROHIBIT is recorded in 4K spte entry. */
			    ...
			    gfn_t gfn, bool *is_private)
{
	...
	for_each_slot_rmap_range(memslot, PG_LEVEL_4K,
		...
		*is_private = is_private_spte(sptep);
		return 0;
	}
