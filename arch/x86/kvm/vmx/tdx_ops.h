				...
				struct tdx_module_output *out)
{
	...
	return seamcall_sept_retry(TDH_MEM_SEPT_ADD, gpa | level, tdr, page, 0, out);
}
...
static inline u64 tdh_mem_range_block(hpa_t tdr, gpa_t gpa, int level,
				struct tdx_module_output *out)
{
	return seamcall_sept_retry(TDH_MEM_RANGE_BLOCK, gpa | level, tdr, 0, 0, out);
}
...
static inline u64 tdh_mem_page_remove(hpa_t tdr, gpa_t gpa, int level,
				struct tdx_module_output *out)
{
	return seamcall_sept_retry(TDH_MEM_PAGE_REMOVE, gpa | level, tdr, 0, 0, out);
}
...
static inline u64 tdh_mem_range_unblock(hpa_t tdr, gpa_t gpa, int level,
					struct tdx_module_output *out)
{
	return seamcall_sept_retry(TDH_MEM_RANGE_UNBLOCK, gpa | level, tdr, 0, 0, out);
}
