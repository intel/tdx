					...
					unsigned long tdmr_end_pfn)
{
	...
	pamt_alloc_start = tdmr_start_pfn << PAGE_SHIFT;
	if (pamt_alloc_start < tdx_cmrs[0].base)
		pamt_alloc_start = tdx_cmrs[0].base;
...
static int __init sanitize_cmrs(void)
{
	...
	for (i = 0; i < e820_table->nr_entries; i++) {
		...
		/*
		 * Skip the memory below 1MB, which is not included in CMRs,
		 * otherwise below check will fail.
		 */
		if (entry->addr + entry->size <= SZ_1M)
			continue;
...
static int __init build_tdsysinfo_and_cmrs_from_e820(void)
{
	...
	for (i = 0, j = 0; i < e820_table->nr_entries; i++) {
		...
		/*
		 * Skip memory below 1MB, since they won't be included in real
		 * CMRs generated by BIOS.
		 */
		if (entry->addr + entry->size <= SZ_1M)
			continue;
