				     ...
				     struct tdp_iter *iter, u64 new_spte)
{
	...
	struct kvm *kvm = vcpu->kvm;
	struct tdp_iter child_iter; /* Don't modify iter. */
	...
	/*
	 * Step down to spte.  Because tdp_iter_next() assumes the spte isn't
	 * freezed, manually does it.
	 */
	child_iter = *iter;
	/* Adjust target gfn to the head gfn of large page. */
