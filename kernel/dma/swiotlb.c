...
static __init int setup_areas(int n)
{
	...
	if ((default_nslabs / n) % IO_TLB_SEGSIZE)
		n = default_nslabs / IO_TLB_SEGSIZE;
	pr_info("swiotlb: Using %d areas\n", n);
	num_areas = n;
	area_index_shift = __fls(default_nslabs) - __fls(num_areas);
	area_bits = __fls(n);
	return 0;
}
...
{
	struct io_tlb_mem *mem = (struct io_tlb_mem *)mem;
	*val = mem_used(mem);
	return 0;
}
