...
static ssize_t tdx_keyid_num_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "0x%08x", tdx_keyid_num);
}

static struct kobj_attribute tdx_keyid_num_attr = {
	.attr = { .name = "keyid_num", .mode = 0444 },
	.show = tdx_keyid_num_show,
};
...
static int __init tdx_sysfs_init(void)
{
	...
	ret = sysfs_create_file(tdx_kobj, &tdx_keyid_num_attr.attr);
	if (ret) {
		pr_err("Sysfs exporting seam keyid_num failed %d\n", ret);
		return ret;
	}
