					    ...
					    gfn_t gfn, int count)
{
	...
	for (i = PG_LEVEL_2M; i <= KVM_MAX_HUGEPAGE_LEVEL; ++i) {
		...
		WARN_ON_ONCE(disallow_count + count < 0 ||
						      ...
						      union kvm_mmu_page_role role)
{
	...
	/*
	 * kvm_mmu_alloc_private_spt() requires valid role.level is set.
	 * Allocate it later.
	 */
	...
	if (is_private)
		kvm_mmu_alloc_private_spt(vcpu, caches->private_spt_cache, sp);
				   ...
				   struct kvm_page_fault *fault)
{
	...
	/* Fault is shared, fall back to the existing path. */
	return RET_PF_CONTINUE;
}
...
static void kvm_mmu_zap_memslot(struct kvm *kvm, struct kvm_memory_slot *slot)
{
	...
	if (is_tdp_mmu_enabled(kvm)) {
		...
		if (kvm_gfn_shared_mask(kvm)) {
			...
			if (flush)
				kvm_flush_remote_tlbs(kvm);
			flush = slot_handle_level(kvm, slot, kvm_drop_private_zapped_rmapp,
