...
static void tdx_trace_seamcalls(u64 level)
{
	...
	ret = seamcall(SEAMCALL_TDDEBUGCONFIG, DEBUGCONFIG_SET_TRACE_LEVEL, level,
		0, 0, NULL, NULL);
	WARN_ON_ONCE(ret);
}
...
P_SEAMLDR_ATTR_SHOW(seam_ready, "0x%02x");
P_SEAMLDR_ATTR_SHOW(seam_debug, "0x%02x");
P_SEAMLDR_ATTR_SHOW(p_seamldr_ready, "0x%02x");

static ssize_t p_seamldr_seaminfo_show(
	struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	BUILD_BUG_ON(sizeof(p_seamldr_info.seaminfo) > PAGE_SIZE);
	memcpy(buf, p_seamldr_info.seaminfo, sizeof(p_seamldr_info.seaminfo));
	return sizeof(sizeof(p_seamldr_info.seaminfo));
}
static struct kobj_attribute p_seamldr_seaminfo = {
	.attr = { .name = "seaminfo", .mode = 0444 },
	.show = p_seamldr_seaminfo_show,
};
...
static struct attribute *p_seamldr_attrs[] = {
	...
	&p_seamldr_seaminfo.attr,
	&p_seamldr_seam_ready.attr,
	&p_seamldr_seam_debug.attr,
	&p_seamldr_p_seamldr_ready.attr,
	NULL,
};
...
TDX_MODULE_ATTR_SHOW(attributes_fixed0, "0x%016llx");
TDX_MODULE_ATTR_SHOW(attributes_fixed1, "0x%016llx");
TDX_MODULE_ATTR_SHOW(xfam_fixed0, "0x%016llx");
TDX_MODULE_ATTR_SHOW(xfam_fixed1, "0x%016llx");
