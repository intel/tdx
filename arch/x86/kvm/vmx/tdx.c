...
static void tdx_mmu_release_hkid(struct kvm *kvm)
{
...
static void tdx_mmu_notifier_release(struct mmu_notifier *mn,
				     struct mm_struct *mm)
{
	struct kvm_tdx *kvm_tdx = container_of(mn, struct kvm_tdx, mmu_notifier);
...
static const struct mmu_notifier_ops tdx_mmu_notifier_ops = {
	.release = tdx_mmu_notifier_release,
};

static int tdx_init_mmu_notifier(struct kvm *kvm)
{
	struct kvm_tdx *kvm_tdx = to_kvm_tdx(kvm);

	kvm_tdx->mmu_notifier.ops = &tdx_mmu_notifier_ops;
	return mmu_notifier_register(&kvm_tdx->mmu_notifier, current->mm);
}
...
void tdx_vm_free(struct kvm *kvm)
{
	...
	if (kvm_tdx->mmu_notifier.ops)
		mmu_notifier_unregister(&kvm_tdx->mmu_notifier, kvm->mm);
