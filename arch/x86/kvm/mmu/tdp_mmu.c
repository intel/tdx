				     ...
				     struct tdp_iter *iter, u64 new_spte)
{
	...
	if (ret)
		goto out;
	kvm_flush_remote_tlbs_range(kvm, gfn, KVM_PAGES_PER_HPAGE(level));
					  ...
					  struct tdp_iter *iter)
{
	...
	if (unlikely(!fault->slot))
		...
		if (kvm_gfn_shared_mask(vcpu->kvm) && !fault->private)
...
int kvm_tdp_mmu_map(struct kvm_vcpu *vcpu, struct kvm_page_fault *fault)
{
	...
	struct kvm_mmu_page *sp;
	gfn_t raw_gfn;
	bool is_private = fault->private && kvm_gfn_shared_mask(kvm);
